...
public class EtlRocketRecordReader extends RecordReader<EtlKey, CamusWrapper> {
	  ...
	  EtlRocketSplit split;
	  private static Logger log = Logger.getLogger(EtlRocketRecordReader.class);
	   ...
	   */
	  public EtlRocketRecordReader(RocketInputFormat inputFormat, InputSplit split, TaskAttemptContext context) 
	...
	public void initialize(InputSplit split, TaskAttemptContext context) 
	    ...
	    this.split = (EtlRocketSplit) split;
	...
	public boolean nextKeyValue() throws IOException, InterruptedException {
		if(System.currentTimeMillis()>maxPullTime){
		    ...
		    while((request = (EtlRequest)this.split.popRequest())!=null){
	    ...
	    while(true){
	    	try{
	    		if(reader==null||!reader.hadNext()){
	    			RocketRequest request = (RocketRequest)this.split.popRequest();
	    			if(request==null){
	    				return false;
	    			}
	    			log.info("recordRead request:"+request.toString());
	    			if(maxPullHours>0){
	    				endTimeStamp = 0;
	    			}
	    			 ...
	    			 key.set(request.getTopic(), request.getLeaderId(), request.getPartition(), request.getOffset(),
    		          ...
    		          reader = new RocketReader(inputFormat,context, request,CamusJob.getKafkaTimeoutValue(mapperContext),CamusJob.getKafkaBufferSize(mapperContext));
	    			...
	    			context.progress();
	    			 mapperContext.getCounter("total", "data-read").increment(msgValue.getLength());
	    	          ...
	    	          if (wrapper == null) {
	    	            ...
	    	            log.info("mapper value:"+wrapper.getRecord());
	    	            value = wrapper;
	...
	private CamusWrapper getWrappedRecord(String topicName, byte[] payload) throws IOException {
	    ...
	    try {
	      ...
	      mapperContext.getCounter(ROCKETMSG.DECODE_SUCCESSFUL).increment(1);
	    } catch (SchemaNotFoundException e) {
	      mapperContext.getCounter(ROCKETMSG.SKIPPED_SCHEMA_NOT_FOUND).increment(1);
	      if (!skipSchemaErrors) {
	        throw new IOException(e);
	      }
	    } catch (Exception e) {
	      mapperContext.getCounter(ROCKETMSG.SKIPPED_OTHER).increment(1);
	      if (!skipSchemaErrors) {
	        throw new IOException(e);
	      }
	    }
