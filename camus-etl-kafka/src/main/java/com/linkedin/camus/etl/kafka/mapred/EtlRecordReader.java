    ...
    while (true) {
      try {
        ...
        while (reader.getNext(key, msgValue, msgKey)) {
          ...
          try {
//        	this.context
        	String splitField = context.getConfiguration().get("split");
            String format = context.getConfiguration().get("format");
            if(splitField!=null&&splitField.length()>0){
        		key.setTime(this.getMsgMils(wrapper,splitField,format));
        	}else{
        	...
        	log.error(wrapper.getRecord().toString());
            mapperContext.write(key, new ExceptionWritable(e));
            continue;
          }
...
private Long getMsgMils(CamusWrapper wrapper,String splitField,String format) {
	StringTokenizer st = new StringTokenizer(wrapper.getRecord().toString(),",",false);
	Long msgMils = wrapper.getTimestamp();
    try{
        while (st.hasMoreTokens()){
            String time  =st.nextToken();
            if(time!=null&&time.contains(splitField)){
                String timeReplace = time.replaceAll("\"","");
                String key = timeReplace.substring(0,timeReplace.indexOf(":"));
                String value = timeReplace.substring(timeReplace.indexOf(":")+1);
                if(key.equals(splitField)){
                    if(format!=null&&format.length()>0){
                        try{
                            msgMils = new SimpleDateFormat(format.replaceAll("\"","")).parse(value).getTime();
                        }catch(Throwable ex){
                            ex.printStackTrace();
                        }
                    }else{
                            msgMils = Long.valueOf(value);
                    }
                    break;
                }
            }
